...
def grad_cam(model, img_array, layer_name):
    ...
    with tf.GradientTape() as tape:
        ...
        loss = predictions[0]  # Sigmoid output (single value for tumor)
    ...
    cam = cv2.resize(cam, (img_array.shape[2], img_array.shape[1]))  # Match input image dims
    cam = cam / np.max(cam)
    return cam
...
if uploaded_files:
    for uploaded_file in uploaded_files:
        ...
        # Display prediction and original image
        ...
        if prob > 0.5:
            layer_name = 'conv5_block3_out'  # Last ResNet50 conv layer
            try:
